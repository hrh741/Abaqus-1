...
def modelVCCTUEL(model1,ls,ws,plies,halfStructure,EquationOrDisplacement,ex=0.1,cracks=[]):
    """
    利用VCCT计算能量释放率分量
    创建edge delaminate  laminate的计算job
    
    Parameters
    ----------
    ls: tuple 
        (层合板长度,长度方向的单元数目,double seed 的长度比例)
        (180.0,20,1.0)
    ws: tuple 
        (层合板宽度,宽度方向上的单元数目,double seed 的长度比例)
        (25,100,1.0)
    plies: list
        层合板铺层信息列表，每一项都分别表示(铺排角（度数），厚度，材料名，厚度方向单元数目，double seed的比例)
        [(0,     0.135,    "T300-7901",  8,    1.0),]
    halfStructure: 长度为3的列表
        分别表示是否在x,y,z方向上使用对称 0不使用 1使用对称
        [0,0,1] #使用zsymm
    EquationOrDisplacement: 0 | 1
        0 表示 使用Equation 约束
        1 表示 使用边界条件x=0完全固定,x=1处u1=constant
    ex:
        轴向应变
    cracks:
        端部裂纹的信息
        [(5,2.0,0)] 在第5+1=6层和第5+2层之间插入长度为2.0的表示从左端向中间延伸的边界裂纹，
        [(5,2.0,1)] 在第5+1=6层和第5+2层之间插入长度为2.0的从右端向中间延伸的边界裂纹
    ...
    for crack_ind,crack in enumerate(cracks):
        ...
        if cracktype==0:
            ...
            "crack 到y=lam_w结束"
            ymin,ymax=lam_wid-cracklen-ACISEPS,lam_wid-cracklen+w/ydiv+ACISEPS
            ns=myInstance1.nodes.getByBoundingBox(zMin=zmin,zMax=zmax,yMin=ymin,yMax=ymax)
            assert len(ns)==8,"crack tip bounding box error" 
            ymid=(ymin+ymax)/2.0
            xmid=lam_len/2.0
...
if __name__=="__main__":
    for i in range(1,10):
        ...
        crack_len=0.05*i
        cracks=[(2,crack_len,0),]
        
        model1=mdb.Model(name="VCCT_0_+-30_+-60_90s-30_-30-1000a=%d"%(int(1000*crack_len)))
